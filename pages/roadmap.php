...
<nav class="main-menu">
  <ul>
    ...
    <?php
       include("./settings/connection.php");
       $roadmaps = mysqli_query($conn,"SELECT * FROM `roadmaps`");
       while($roadmaprow = mysqli_fetch_array($roadmaps)) {
          echo '<li class="has-subnav">
          <a href="index.php?page=roadmap&id='.$roadmaprow["id"].'">
          <i class="fa fas fa-level-up-alt fa-rotate-90 fa-1.5x"></i>
          <span class="nav-text">
          '. $roadmaprow["name"] .'
          </span>
          </a>
...
<div class="timelayn">
  <h1 class="center"><?php 
     ...
     $query = mysqli_query($conn,"SELECT * FROM `roadmaps` LIMIT 1");
     $row = mysqli_fetch_array($query);
     $id = $row["id"];
    }
    $roadMapNameSql = mysqli_query($conn,"SELECT * FROM `roadmaps` WHERE `id`='$id'");
    $roadMap = mysqli_fetch_array($roadMapNameSql);
    echo $_SESSION["username"] . "'s " . $roadMap["name"];
  ?> Roadmap</h1>
  <ul class="timeline">
    <?php   
      $yapici = mysqli_query($conn, "SELECT * FROM `roadmaps` WHERE `id`='$id'");
      $yapicirow = mysqli_fetch_array($yapici);
      $yapmayiciid = $yapicirow["id"];
      $yapmayici = mysqli_query($conn, "SELECT * FROM `altroadmaps` WHERE `roadmap_id`='$yapmayiciid' ORDER BY `row_number`");
      $a = 0;
      while($yapmayicirow = mysqli_fetch_array($yapmayici)) {
        global $a; 
        $a +=1;
        if($a % 2==0) {
          $b = "direction-r";
        ...
        '
        <li>
        <a href="'. $yapmayicirow["content"] .'" target="_blank">
          <div class="'. $b .'">
            <div class="flag-wrapper">
              <span class="">'. $yapmayicirow["name"] .'</span>
              <span class="time-wrapper">
              </span>
            </div>
            <div class="desc">'. $yapmayicirow["short_content"] .'</div>
          </div>
          </a>
        </li>
        ';
